...
#define DT_DRV_COMPAT microchip_pic32ck_eth

/* Map SAM Kconfig names used by eth_sam_gmac_priv.h to PIC32CK equivalents */
#define CONFIG_ETH_SAM_GMAC_QUEUES        CONFIG_ETH_PIC32CK_GC_GMAC_QUEUES
#define CONFIG_ETH_SAM_GMAC_BUF_RX_COUNT  CONFIG_ETH_PIC32CK_GC_GMAC_BUF_RX_COUNT
...
/* PIC32CK: no atmel_sam_pmc needed */
#include <soc.h>
...
static void queue0_isr(const struct device *dev)
{
	...
	/* PIC32CK: ISR is write-1-to-clear (NOT read-clear like SAM GMAC) */
	isr = gmac->GMAC_ISR;
	gmac->GMAC_ISR = isr;
	gmac->GMAC_RSR = gmac->GMAC_RSR;
...
#elif defined(CONFIG_SOC_SAMA7G54)
#elif defined(CONFIG_SOC_FAMILY_MICROCHIP_PIC32CK_SG_GC)
	/* GCLK, ETH wrapper, and MDIO are already enabled by the MDIO driver
	 * init (which runs earlier due to PHY dependency). Nothing to do here.
	 */
#else
...
static void eth_iface_init(struct net_if *iface)
{
	...
	if (result < 0) {
		dev_data->mac_addr[0] = 0xD8;
		dev_data->mac_addr[1] = 0x80;
		dev_data->mac_addr[2] = 0x39;
		dev_data->mac_addr[3] = 0x12;
		dev_data->mac_addr[4] = 0x34;
		dev_data->mac_addr[5] = 0x56;
	}
	...
	if (device_is_ready(cfg->phy_dev)) {
		...
		/* Check if link is already up (race condition workaround) */
		{
			struct phy_link_state state = {0};
			if (phy_get_link_state(cfg->phy_dev, &state) == 0 &&
			    state.is_up) {
				phy_link_state_changed(cfg->phy_dev, &state,
						       (void *)dev);
			}
		}
